...
def serialize(steps: list) -> str:
    ...
    Each step becomes one line: ``movej j1 j2 j3 j4 j5 j6 vel`` or
    ``movel x y z rx ry rz vel``.  Fields are space-separated floats.
    ...
    for step in steps:
        if isinstance(step, MoveJStep):
            lines.append(
                ...
                f" {step.j4} {step.j5} {step.j6} {step.vel}"
            )
        elif isinstance(step, MoveLStep):
            lines.append(
                f"movel {step.x} {step.y} {step.z}"
                f" {step.rx} {step.ry} {step.rz} {step.vel}"
            )
        else:
...
def deserialize(text: str) -> list:
    ...
    or ``movel`` followed by exactly 7 space-separated float fields.

    @param text<str>: Text produced by serialize() or compatible format.
    @return<list>: Ordered list of MoveJStep / MoveLStep instances.
    @raises ValueError: If a line has an unknown kind or wrong field count,
        or if a field cannot be parsed as float.
    """
    steps = []
    for lineno, raw in enumerate(text.splitlines(), start=1):
        ...
        if len(parts) != 8:
            raise ValueError(
                f"Line {lineno}: {kind} expects 7 fields,"
                f" got {len(parts) - 1}"
            )
        try:
        ...
        if kind == "movej":
            steps.append(MoveJStep(f[0], f[1], f[2], f[3], f[4], f[5], f[6]))
        else:
            steps.append(MoveLStep(f[0], f[1], f[2], f[3], f[4], f[5], f[6]))
    return steps
